...
func main() {
	...
	ins.Exec("芝山鉄道線", 1435, 2.2, 2, 10, 9)
	ins.Exec("都営浅草線", 1435, 18.3, 20, 2, 10)

	// trains への insert
	query = "insert into `trains`(name, line_id, operator_id, max_speed, acceleration) values(?, ?, ?, ?, ?)"
	ins, err = db.Prepare(query)
	if err != nil {
		log.Fatalln(err)
	}
	ins.Exec("東急 1000 系", 1, 1, 85, 3.5)
	ins.Exec("東急 1000 系", 2, 1, 85, 3.5)
	ins.Exec("京急 2100 形", 3, 2, 120, 3.5)
	ins.Exec("京急 2100 形", 4, 2, 120, 3.5)
	ins.Exec("京急 2100 形", 5, 2, 120, 3.5)
	ins.Exec("東急 5050 系", 7, 1, 110, 3.3)
	ins.Exec("東急 5050 系", 8, 1, 110, 3.3)
	ins.Exec("東急 5050 系", 9, 1, 110, 3.3)
	ins.Exec("東急 5050 系", 10, 1, 110, 3.3)
	ins.Exec("東急 5050 系", 11, 1, 110, 3.3)
	ins.Exec("東急 5050 系", 12, 1, 110, 3.3)
	ins.Exec("東急 5050 系", 13, 1, 110, 3.3)
	ins.Exec("東急 5050 系", 14, 1, 110, 3.3)
	ins.Exec("東京都交通局 5500 形", 3, 10, 120, 3.3)
	ins.Exec("東京都交通局 5500 形", 4, 10, 120, 3.3)
	ins.Exec("東京都交通局 5500 形", 5, 10, 120, 3.3)
	ins.Exec("東京都交通局 5500 形", 6, 10, 120, 3.3)
	ins.Exec("東京都交通局 5500 形", 15, 10, 120, 3.3)
	ins.Exec("東京都交通局 5500 形", 16, 10, 120, 3.3)
	ins.Exec("東京都交通局 5500 形", 17, 10, 120, 3.3)
	ins.Exec("東京都交通局 5500 形", 18, 10, 120, 3.3)
	ins.Exec("東京都交通局 5500 形", 19, 10, 120, 3.3)
	ins.Exec("東京都交通局 5500 形", 20, 10, 120, 3.3)
	ins.Exec("東京都交通局 5500 形", 21, 10, 120, 3.3)
	ins.Exec("東急 7000 系", 1, 1, 85, 3.3)
	ins.Exec("東急 7000 系", 2, 1, 85, 3.3)
}
